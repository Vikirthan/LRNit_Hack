...
serve(async (req) => {
  ...
  try {
    const { email, name, subject, content, signature, fromEmail, fromName } = await req.json();
      ...
      body: JSON.stringify({
        ...
        htmlContent: `
          <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: auto; padding: 30px; border: 1px solid #e2e8f0; border-radius: 16px; background-color: #ffffff;">
            ...
            <div style="font-size: 15px; color: #475569; line-height: 1.8; margin: 20px 0; white-space: pre-wrap;">
              ${content}
            </div>
