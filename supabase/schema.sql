-- Supabase schema for TicketScan
...
begin
  ...
  if team_row.active_out is not null then
    raise exception 'Team is already OUT';
  end if;

  v_out_payload := jsonb_build_object(
    'out_at', now(),
    'members_out', p_members_out,
    'actor_uid', p_actor_id
  );

  update public.teams
  set active_out = v_out_payload,
      updated_at = now()
  where team_id = p_team_id;

  insert into public.break_sessions (team_id, members_out, out_at, out_by, status)
  values (p_team_id, p_members_out, now(), p_actor_id, 'active');

  insert into public.scan_logs (team_id, action_type, payload, actor_id)
  values (p_team_id, 'OUT', v_out_payload, p_actor_id);
...
begin
  ...
  if team_row.active_out is null then
    raise exception 'Team is not currently OUT';
  end if;
  ...
  v_in_payload := jsonb_build_object(
    'in_at', now(),
  ...
  update public.teams
  set active_out = null,
      penalty_points = penalty_points + v_penalty,
      updated_at = now()
  where team_id = p_team_id;
  ...
  insert into public.scan_logs (team_id, action_type, payload, actor_id)
  values (p_team_id, 'IN', v_in_payload, p_actor_id);
...
begin
  update public.teams
  set penalty_points = penalty_points + p_delta,
      updated_at = now()
  where team_id = p_team_id;

  insert into public.penalty_adjustments (team_id, delta, reason, actor_id)
  values (p_team_id, p_delta, p_reason, p_actor_id);
...
begin
  ...
  insert into public.scan_logs (team_id, action_type, payload, actor_id)
  values (p_team_id, 'reset_penalty', '{}'::jsonb, p_actor_id);
end;
$$;
...
values
  ('vikirthan', crypt('Vikirthan@819', gen_salt('bf')), 'admin', 'Vikirthan', 'vikirthan@ticketscan.local', 'approved'),
  ('teacher', crypt('Teacher@819', gen_salt('bf')), 'teacher', 'Teacher', 'teacher@ticketscan.local', 'approved'),
  ('volunteer', crypt('Volunteer@819', gen_salt('bf')), 'volunteer', 'Volunteer', 'volunteer@ticketscan.local', 'approved')
