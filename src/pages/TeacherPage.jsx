...
export default function TeacherPage() {
  ...
  const total = useMemo(
    () =>
      TEACHER_CRITERIA.reduce((sum, item) => sum + clampScore(effectiveScoreByCriterion[item.key], item.max), 0),
  ...
  const saveScore = async () => {
    try {
      ...
      const payload = {
        team,
        scoreByCriterion: effectiveScoreByCriterion,
        remarks,
        total,
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '12px 24px' }}>
                  {TEACHER_CRITERIA.map(crit => (
                    <div key={crit.key} style={{ display: 'contents' }}>
                      ...
                      <strong style={{ color: '#fff', fontSize: '0.95rem', textAlign: 'right' }}>{effectiveScoreByCriterion[crit.key]} / {crit.max}</strong>
                    </div>
                  ))}
